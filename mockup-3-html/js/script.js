/* ============================================================
   ...
   Scroll reveals, counters, marquee, and the WhatsApp front-desk
   bot. Plain script, no modules.
   ============================================================ */
(function () {
  'use strict';
  ...
  const FLOW = {
    start: {
      bot: ["Welcome to Victoriashire. You've reached our admissions desk.", 'How may we help you today?'],
      options: [
        { label: 'Programmes & ages', next: 'programmes' },
        { label: 'Book a private tour', next: 'visit' },
    ...
    programmes: {
      bot: [
        'We educate children at every stage of the early years:',
        'Early Childhood Education — 4 months to 6 years\nCambridge English — ages 4 to 12\nVIS Music School — ages 1 to 18\nDaycare & Tuition — ages 7 to 12',
      ],
      options: [
        { label: 'View programmes', act: 'go:#programmes' },
        { label: 'Back', next: 'start' },
      ],
    },
    visit: {
      bot: ['We would be glad to show you a morning in session.', 'Choose a day and our admissions team will meet you at reception.'],
      options: [
        { label: 'Admissions details', act: 'go:#visit' },
        { label: 'WhatsApp admissions', act: 'whatsapp' },
        { label: 'Back', next: 'start' },
      ],
    },
    location: {
    ...
    more: {
      bot: ['Is there anything else we can help you with?'],
      options: [
  ...
  panel.innerHTML =
    '<div class="wa-head">' +
      '<span class="wa-avatar"><img src="img/logo-preschool.avif" alt=""><span class="wa-dot"></span></span>' +
      '<span class="wa-head-txt"><strong>Victoriashire Admissions</strong><small>Typically replies instantly</small></span>' +
  ...
  const runAct = (act) => {
    if (act === 'whatsapp') window.open('https://wa.me/' + WHATSAPP, '_blank', 'noopener');
      ...
      toggle();
      const target = document.querySelector(act.slice(3));
      if (target) setTimeout(() => target.scrollIntoView({ behavior: REDUCE ? 'auto' : 'smooth' }), 250);
    }
  };
